...
def test_chunk_text_basic(text_chunker):
    ...
    assert "short sentence. And a third one." in chunks[1] # The second chunk starts from where the first ended
...
def test_chunk_text_with_overlap(text_chunker):
    ...
    assert text_chunker.tokenizer.decode(text_chunker.tokenizer.encode(text)[10:15]) in chunks[0] # Part of overlap in first chunk
    assert text_chunker.tokenizer.decode(text_chunker.tokenizer.encode(text)[10:15]) in chunks[1] # Part of overlap in second chunk
...
def test_chunk_text_exact_fit(text_chunker):
    ...
    assert len(chunks) == 2
    assert text_chunker.tokenizer.decode(text_chunker.tokenizer.encode(text_exact)[:6]) == chunks[0]
    assert text_chunker.tokenizer.decode(text_chunker.tokenizer.encode(text_exact)[6:]) == chunks[1]
