...
def test_embed_empty_string(embedding_model):
    ...
    assert len(embedding) > 0 # Sentence-transformers usually returns a vector for empty string
...
def test_embed_invalid_input_type(embedding_model):
    ...
    with pytest.raises(TypeError, match="Input 'texts' must be a string or a list of strings."):
