...
def temp_chroma_db(tmp_path):
    ...
    vs = VectorStore(collection_name=collection_name, persist_directory=str(persist_directory))
    yield vs
...
def test_add_documents(temp_chroma_db):
    ...
    retrieved = collection.get(ids=["id1", "id2", "id3"], include=['documents', 'metadatas'])
...
def test_add_documents_mismatched_lengths(temp_chroma_db):
    ...
    with pytest.raises(ValueError, match="Lengths of documents, embeddings, metadatas, and ids must match."):
        temp_chroma_db.add_documents(documents, embeddings)

def test_query(temp_chroma_db):
    """Tests querying the vector store."""
    documents = ["apple pie is delicious", "banana split is tasty", "cherry tart is sweet"]
